...
("selop[1..0]", "output", ur"Índex d'operació que s'està efectuant"),
...
al mode d'introducció prement una de les \emph{tecles de sel·lecció d'operació} i al mode
de visualització prement \texttt{\#}.
'''
...
Un biestable D (amb habilitació de càrrega controlada per $ast$) memoritza l'últim valor
de $selop$, l'operació a realitzar.

Els factors es retornen i també es porten a \textsf{Calculadora} juntament amb $selop$ per
a efectuar l'operació corresponent i s'obté el resultat, que es fa passar per \textsf{sel}.
El resultat final ($sigSel$, $sel$) es porta a uns biestables
D abans de ser retornat, la qual cosa garantitza una sortida neta d'espuris.
'''
